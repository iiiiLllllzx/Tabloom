...
- 一次处理所有普通 Chrome 窗口，按域名前两段自动归并和着色，例如 `ml.bytedance.net → ml-bytedance`。
- 每个窗口独立建组和计数，整理所有窗口时不会跨窗口移动或合并标签。
- 支持 `2–20` 的分组阈值；同域名标签不足阈值时保持未分组并移动到窗口最右侧。
- 自动组充分轮用 Chrome 原生九色，并以高对比顺序错开相邻标签组。
- 自动整理新打开的网页，不覆盖已有组或手工分组。
- 手工拖动保护按标签生效：只跳过被拖动的标签，同组其他标签仍参与阈值计算和重复组归并。
- 一键取消所有窗口的标签组，不改变自动整理开关。
...
- 弹窗和工作台中的“按域名整理”会一次处理所有普通窗口，并使用域名前两段生成短组名，例如 `code.byted.org → code-byted`。
- 默认阈值为 3；只有同一窗口内同域名标签达到阈值才分组，未达到阈值的标签会取消自动组并移到该窗口最右侧。
- 关闭组内标签后会自动重新计算原窗口；剩余数量低于阈值时立即取消该自动组。
- Chrome 原生标签组只提供九种颜色。Tabloom 按标签栏顺序轮用全部九色，避免相邻组出现同色或红/粉、蓝/青等近似色；再次整理会更新已有自动组颜色。
- 旧版完整域名组会在下次整理时自动合并和重命名，手工分组仍会保留。
- 同窗口出现重复的自动域名组时，未手工标记的标签会归并到同一个组；手工标记标签保持原位。
- 自动整理开启后，新网页会进入对应域名组。
- 已有原生标签组、固定标签和手工拖拽结果不会被自动整理覆盖。
- 工作台中的自定义空组是临时拖放目标；拖入第一个标签时才创建 Chrome 原生组。
- Chrome 原生标签组不能跨窗口；Tabloom 创建组时显式锁定原窗口，工作台顶部可以切换管理窗口。
- “撤销”逐步回退本会话的分组操作；“恢复分组”回到 Tabloom 首次修改该窗口之前的状态。
- “取消分组”作用于所有窗口的全部标签组，但不会关闭自动整理；之后新建或更新标签时仍会按阈值重新整理。可切换到对应窗口后使用“撤销”恢复。
