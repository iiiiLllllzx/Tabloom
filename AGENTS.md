...
- 手工覆盖以 `tabId` 为粒度；自动整理只跳过被标记标签，同组其他未标记标签仍须参与自动组识别、阈值计算和重复组归并。
...
- 用户主动“按域名整理”必须遍历所有普通 Chrome 窗口；“取消分组”也作用于所有普通窗口，但不得修改 `autoGroupEnabled`。
- 设置 schema v3 会将旧版因“取消分组”遗留的关闭状态迁移为开启；v3 中用户主动关闭的选择必须保留。
- 新建自动组必须传入 `createProperties.windowId`，禁止 Chrome 将其他窗口标签移动到当前窗口；窗口之间始终独立。
- 同一窗口的自动整理必须串行执行，避免后台事件与按钮操作并发创建同名组。
- `tabs.onRemoved` 必须在非窗口关闭场景重新调度 `removeInfo.windowId`，保证剩余标签低于阈值时自动解组。
