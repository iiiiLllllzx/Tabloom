...
| `color-service` | 使用全部 Chrome 原生九色，为按标签位置排序的自动组生成相邻高对比配色计划 |
| `group-history` | 捕获窗口分组快照，维护 20 步撤销栈和初始恢复点 |
...
默认开启自动整理，分组阈值为 3，允许配置范围为 `2–20`。每个窗口独立计数。
...
该数据同样按浏览器会话保存。覆盖标记严格按 `tabId` 生效：自动整理只跳过对应标签，不得因组内存在一个标记而跳过整个组；用户主动点击“重新按域名整理”时可清除当前窗口的覆盖标记。
...
1. 查询目标窗口的全部标签和已有标签组。
2. 排除固定标签、受限 URL、无 `tabId` 标签和手工标记标签；已识别自动组中的其他未标记标签继续参与。
3. 用主机名前两段生成短域名键，例如 `ml.bytedance.net → ml-bytedance`。
4. 每个窗口独立比较桶大小和阈值；达到阈值则分组，不足阈值则取消自动组并调用 `tabs.move({ index: -1 })` 移到窗口最右侧。
5. 识别并合并旧版或重复自动组中的未标记标签；同组手工标记标签保持原位。
6. 修改前保存窗口快照，优先复用对应短域名组；否则调用 `tabs.group({ tabIds, createProperties: { windowId } })` 在原窗口创建组，防止跨窗口移动标签。
7. 用户主动整理遍历全部普通 Chrome 窗口；每个窗口通过串行队列避免按钮与后台事件并发创建重复组。
8. `tabs.onCreated`、`tabs.onUpdated` 与非窗口关闭的 `tabs.onRemoved` 仅重新计算事件所在窗口，使用短延迟合并重复事件；关闭标签导致组数量低于阈值时自动解组。
...
- 颜色映射：确定性、颜色枚举合法性、前九组不重复、相邻组无同色或低对比色对。
