...
4. 识别并合并旧版完整域名/单段域名自动组，明确手工分组保持不变。
5. 修改前保存窗口快照，优先复用对应短域名组；否则调用 `tabs.group` 创建组。
6. 调用 `tabGroups.update` 设置短标题和稳定颜色。
7. `tabs.onCreated` 与 `tabs.onUpdated` 仅在 URL 稳定且自动开关开启时调度，使用短延迟合并重复事件。
